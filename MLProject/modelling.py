...
def train_model(df, target_column='diagnosis'):
    """
    ...
    with mlflow.start_run(run_name="Basic_LogisticRegression_Run") as run:
        print(f"Memulai MLflow Run ID: {run.info.run_id}")

        # Inisialisasi dan latih model
        model = LogisticRegression(random_state=42, max_iter=1000) # Contoh model
...
if __name__ == "__main__":
    ...
    preprocessed_data_path = "dataset_preprocessing/debernardi_2020_preprocessed.csv"
    ...
    if df_preprocessed is not None:
        ...
        if not df_preprocessed.empty:
            # Melatih model
            run_id = train_model(df_preprocessed.copy())
            if run_id:
